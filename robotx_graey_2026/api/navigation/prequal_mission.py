...
class PrequalMission(Node):
    ...
    def set_mode_guided(self):
        if self.dry:
            ...
            return
        self.mav.mav.command_long_send(1, 1, mavutil.mavlink.MAV_CMD_DO_SET_MODE, 0, mavutil.mavlink.MAV_MODE_FLAG_CUSTOM_MODE_ENABLED, 4, 0, 0, 0, 0, 0)
    ...
    def tick(self):
        ...
        if self.state == S.WAIT_NAV:
            ...
            rgt = self.marker_r + self.r * math.sin(a)
            ...
            if not self.dry:
                for _ in range(10):
                    self.mav.mav.command_long_send(1, 1, mavutil.mavlink.MAV_CMD_COMPONENT_ARM_DISARM, 0, 0, 0, 0, 0, 0, 0, 0)
                    time.sleep(0.1)
